...
| Trajectories split at `R = 0` with an interpolated crossing, and gaps are never bridged | `scene::tests::crossing_traces_are_split_with_an_interpolated_boundary_vertex`, `scene::tests::gaps_break_every_view`, `scene::tests::boundary_samples_belong_to_both_charts_without_interpolation` |
