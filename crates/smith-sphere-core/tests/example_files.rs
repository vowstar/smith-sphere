...
use smith_sphere_core::{Complex, Document, FrequencyUnit, Impedance};
...
fn series_rlc_is_identical_across_ma_ri_db_and_csv() {
    ...
    for (name, text) in [("ma", RLC_MA), ("ri", RLC_RI), ("db", RLC_DB)] {
        let document =
            parse_touchstone(text, &format!("series_rlc_{name}.s1p"), Some(1)).expect("parse");
        assert_matches_model(&document, 0, &frequencies, model);
    }
    let layout = inspect_csv(RLC_CSV, "series_rlc.csv").expect("inspect");
...
fn negative_resistance_file_matches_the_ideal_model() {
    ...
    let document = parse_touchstone(NEGATIVE, "negative_resistance.s1p", Some(1)).expect("parse");
...
fn crossing_tables_match_the_model_and_the_unitless_one_asks_for_a_unit() {
    ...
    let layout = inspect_csv(CROSSING_TSV, "crossing_r_zero.tsv").expect("inspect");
    ...
    let layout = inspect_csv(CROSSING_NO_UNIT, "crossing_no_unit.csv").expect("inspect");
    assert!(layout.needs_unit());
    assert!(build_csv_document(&layout, None, Some(50.0)).is_err());
    let document =
        build_csv_document(&layout, Some(FrequencyUnit::GHz), Some(50.0)).expect("build");
    assert_matches_model(&document, 0, &frequencies, boundary_crossing_impedance);
}
...
fn two_port_file_exposes_both_ports_and_skips_noise_data() {
    ...
    let document = parse_touchstone(TWO_PORT, "two_port_demo.s2p", Some(2)).expect("parse");
...
fn defective_inputs_are_rejected_with_reasons() {
    let error = inspect_csv(MAGNITUDE_ONLY, "magnitude_only.csv").expect_err("magnitude only");
    assert!(error.message.contains("相位"));
    let error = parse_touchstone(V2, "touchstone_v2_unsupported.s1p", Some(1)).expect_err("v2");
